...
class _RhDocumentsScreenState extends State<RhDocumentsScreen> {
        ...
        builder: (ctx, setModalState) => Container(
          ...
          child: SingleChildScrollView(child: Column(
            ...
            children: [
              ...
              Text('Ajouter un document', style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.w700, color: STBColors.primaryBlue)),
              const SizedBox(height: 20),
              TextFormField(controller: titreC, decoration: const InputDecoration(labelText: 'Titre du document')),
              const SizedBox(height: 12),
              TextFormField(controller: descC, decoration: const InputDecoration(labelText: 'Description')),
              const SizedBox(height: 12),
              TextFormField(controller: urlC, decoration: const InputDecoration(labelText: 'URL du document (ou lien OneDrive/Sharepoint)')),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                value: selectedCat,
                decoration: const InputDecoration(labelText: 'Catégorie'),
                items: const [
                  DropdownMenuItem(value: 'RH', child: Text('Ressources Humaines')),
                  DropdownMenuItem(value: 'BANQUE', child: Text('Bancaire')),
                  DropdownMenuItem(value: 'AVANTAGES', child: Text('Avantages & Social')),
              ...
              ElevatedButton(
                onPressed: () async {
                  if (titreC.text.isEmpty || urlC.text.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Titre et URL requis')));
                    return;
                  }
                  final result = await ApiService.post('${ApiConfig.baseUrl}/documents/create.php', {
                    'titre': titreC.text,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Documents Officiels STB', 
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      leading: Container(
                        ...
                        child: const Icon(Icons.picture_as_pdf, color: STBColors.primaryBlue),
                      ),
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(doc['categorie'].toString().toUpperCase(), 
                              style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: STBColors.primaryBlue)),
                          ),
                        ],
                      ),
                      trailing: IconButton(
                        icon: const Icon(Icons.open_in_new, color: STBColors.primaryBlue),
      ...
      floatingActionButton: AuthService.currentUser?.role == 'rh' ? FloatingActionButton.extended(
